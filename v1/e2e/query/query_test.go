...
import (
	...
	"github.com/viant/agently/v1/e2e/internal/harness"
...
func TestTerminalQueryJWTAuthorized(t *testing.T) {
	...
	assert.Equal(t, "user", transcript.Turns[0].Message[0].Role)
}
...
func TestTerminalQueryPDFAttachment(t *testing.T) {
	...
	require.NotEmpty(t, transcript.Turns[0].Message)
	require.NotEmpty(t, transcript.Turns[0].Message[0].Attachment)
	assert.Equal(t, "application/pdf", transcript.Turns[0].Message[0].Attachment[0].MimeType)
...
func transcriptStats(transcript *coresdk.TranscriptOutput) ([]*agconv.ToolCallView, []*agconv.ModelCallView, map[int]struct{}) {
	...
	for _, turn := range transcript.Turns {
		...
		for _, message := range turn.Message {
			...
			if message.Iteration != nil {
				iterations[*message.Iteration] = struct{}{}
			}
			if message.ModelCall != nil {
				modelCalls = append(modelCalls, message.ModelCall)
				if message.ModelCall.Iteration != nil {
					iterations[*message.ModelCall.Iteration] = struct{}{}
				}
			}
			for _, toolMessage := range message.ToolMessage {
				...
				if toolMessage.Iteration != nil {
					iterations[*toolMessage.Iteration] = struct{}{}
				}
				if toolMessage.ToolCall.Iteration != nil {
					iterations[*toolMessage.ToolCall.Iteration] = struct{}{}
				}
			}
		}
	}
...
func writeTranscriptDebug(t *testing.T, transcript *coresdk.TranscriptOutput) {
	t.Helper()
...
func firstLinkedConversationIDFromCoreTranscript(transcript *coresdk.TranscriptOutput) string {
	if transcript == nil {
		return ""
	}
	for _, turn := range transcript.Turns {
...
func collectCoreTranscriptText(transcript *coresdk.TranscriptOutput) string {
	var parts []string
	if transcript == nil {
		return ""
	}
