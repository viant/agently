...
func (a *authExtension) buildOAuthInitiateResponse(r *http.Request) (*oauthInitiateResponse, error) {
	...
	state, err := encryptState(r.Context(), configURL, codeVerifier)
	if err != nil {
...
func (a *authExtension) handleOAuthCallback() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		codeVerifier, err := decryptState(r.Context(), configURL, state)
		if err != nil {
		...
		_, _ = w.Write([]byte(`<html><body><script>try{if(window.opener){window.opener.postMessage({type:'oauth',status:'ok'},'*');}}catch(e){}window.close();</script>Authentication complete.</body></html>`))
	}
}
