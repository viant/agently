...
//go:embed defaults/config.yaml defaults/tools defaults/models defaults/embedders defaults/feeds defaults/knowledge defaults/templates defaults/agents/chatter defaults/agents/coder
...
func SetBootstrapHook() {
	workspace.SetBootstrapHook(func(store *workspace.BootstrapStore) error {
		...
		emptyWorkspace, err := workspace.IsEmptyWorkspaceAt(context.Background(), afs.New(), store.Root())
		if err != nil {
			return err
		}
...
var skipBootstrapDirs = map[string]bool{
	...
	workspace.KindOAuth:    true,
	workspace.KindWorkflow: true,
}
