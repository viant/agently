...
func NewAuthRuntime(ctx context.Context, workspaceRoot string, rt *executor.Runtime) (*authRuntime, error) {
	...
	// Use the shared Datly DAO from the runtime for session persistence.
	dao := rt.DAO
...
type authConfig struct {
	...
	TokenRefreshLeadMinutes int            `yaml:"tokenRefreshLeadMinutes,omitempty" json:"tokenRefreshLeadMinutes,omitempty"` // how early to refresh before expiry (default 15)
		...
		if c, err := r.Cookie(a.cfg.CookieName); err == nil && strings.TrimSpace(c.Value) != "" {
			if sess := a.sessions.Get(r.Context(), strings.TrimSpace(c.Value)); sess != nil {
				if a.requiresOAuthTokens() && !a.ensureSessionOAuthTokens(r.Context(), sess) {
					log.Printf("[v1-auth] session missing usable oauth tokens, invalidating session user=%q", sess.Subject)
...
func (a *authRuntime) requiresOAuthTokens() bool {
...
func (a *authRuntime) ensureSessionOAuthTokens(ctx context.Context, sess *svcauth.Session) bool {
	if sess == nil {
		return false
	}
	if sess.Tokens != nil {
...
// are expired or will expire within the next 15 minutes.
func (a *authRuntime) refreshExpiringSessions(ctx context.Context) {
	...
	horizon := time.Now().Add(a.cfg.tokenRefreshLead())
...
func (a *authRuntime) ensureDefaultUser(w http.ResponseWriter, r *http.Request) *authUser {
	...
	// Never synthesize the default user for OAuth/BFF deployments.
	// This fallback is only valid for explicit local-auth setups.
