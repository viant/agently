...
function normalizeConversationTitle(value = '') {
  const text = normalizeSeedTitle(value);
  if (!text) return '';
...
export function resolveConversationSummary(row = {}) {
  const candidates = [
    row?.Summary,
    row?.summary
  ];
  for (const value of candidates) {
    const summary = normalizeSeedTitle(value);
...
export function resolveConversationTitle(row) {
  ...
  const rawTitle = normalizeConversationTitle(row?.Title || row?.title || '');
