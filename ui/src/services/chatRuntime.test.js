...
describe('createNewConversation', () => {
  ...
  it('clears persisted composer drafts for current and pending conversations on explicit new conversation', async () => {
    ...
    markPendingConversationBootstrap('conv-old');
    ...
    try {
      ...
      expect(hasPendingConversationBootstrap('conv-old')).toBe(false);
    } finally {
      clearPendingConversationBootstrap('conv-old');
      global.window = originalWindow;
    }
  });
});
...
describe('switchConversation', () => {
  ...
  it('releases the switching latch when conversation lookup fails', async () => {
    const conversationState = { values: { id: 'conv-old', queuedTurns: [] } };
    const context = {
      resources: { chat: { activeConversationID: 'conv-old' } },
      Context(name) {
        if (name === 'messages') {
    ...
    client.getConversation.mockRejectedValueOnce(new Error('temporary conversation lookup failure'));

    await expect(enqueueConversationSwitch(context, 'conv-target')).rejects.toThrow('temporary conversation lookup failure');

    expect(context.resources.chat.switchingConversationID).toBe('');
    expect(context.resources.chat.switchQueue).toBeNull();
    expect(context.resources.chat.switchQueueTarget).toBe('');
  });
...
describe('shouldUseLiveStream', () => {
  ...
  it('refines terminal canonical state without destructive reset', async () => {
