...
The portable catalog accepts the generic `workspace-primary` family role.
Agently does not map that role to a vendor font or bundle font files. The
authenticated workspace stylesheet generated by Core registers the role and
its same-origin WOFF2 faces; Agently applies the resulting family to the shell,
