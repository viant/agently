...
func (s *ScyRefreshStore) key() (*kms.Key, error) {
    b, err := s.kp.Key()
    if err != nil {
        return nil, err
    }
    return &kms.Key{Kind: "raw", Raw: string(blowfish.EnsureKey(b))}, nil
}
...
func (s *ScyRefreshStore) Get(k Key) (Refresh, bool, error) {
    ...
    secret, err := s.svc.Load(context.Background(), &scy.Resource{URL: url})
    if err != nil {
...
func (s *ScyRefreshStore) Set(k Key, r Refresh) error {
    ...
    secret := &scy.Secret{URL: url, Key: key}
    secret.SetTargetJSON(payload)
