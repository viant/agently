...
`,"getStyles"),Et=kt,Mt=d(t=>{const a=[...t.values()].reduce((s,g)=>s+g,0),f=[...t.entries()].map(([s,g])=>({label:s,value:g})).filter(s=>s.value/a*100>=1);return vt().value(s=>s.value).sort(null)(f)},"createPieArcs"),Rt=d((t,a,f,S)=>{var O;z.debug(`rendering pie chart
`+t);const s=S.db,g=it(),l=st(s.getConfig(),g.pie),e=40,n=18,o=4,p=450,h=p,v=lt(a),c=v.append("g");c.attr("transform","translate("+h/2+","+p/2+")");const{themeVariables:i}=g;let[x]=ot(i.pieOuterStrokeWidth);x??(x=2);const w=l.textPosition,m=Math.min(h,p)/2-e,D=_().innerRadius(0).outerRadius(m),$=_().innerRadius(m*w).outerRadius(m*w);c.append("circle").attr("cx",0).attr("cy",0).attr("r",m+x/2).attr("class","pieOuterCircle");const u=s.getSections(),A=Mt(u),C=[i.pie1,i.pie2,i.pie3,i.pie4,i.pie5,i.pie6,i.pie7,i.pie8,i.pie9,i.pie10,i.pie11,i.pie12];let b=0;u.forEach(r=>{b+=r});const N=A.filter(r=>(r.data.value/b*100).toFixed(0)!=="0"),k=ct(C).domain([...u.keys()]);c.selectAll("mySlices").data(N).enter().append("path").attr("d",D).attr("fill",r=>k(r.data.label)).attr("class","pieCircle"),c.selectAll("mySlices").data(N).enter().append("text").text(r=>(r.data.value/b*100).toFixed(0)+"%").attr("transform",r=>"translate("+$.centroid(r)+")").style("text-anchor","middle").attr("class","slice");const j=c.append("text").text(s.getDiagramTitle()).attr("x",0).attr("y",-400/2).attr("class","pieTitleText"),G=[...u.entries()].map(([r,M])=>({label:r,value:M})),E=c.selectAll(".legend").data(G).enter().append("g").attr("class","legend").attr("transform",(r,M)=>{const I=n+o,H=I*G.length/2,J=12*n,K=M*I-H;return"translate("+J+","+K+")"});E.append("rect").attr("width",n).attr("height",n).style("fill",r=>k(r.label)).style("stroke",r=>k(r.label)),E.append("text").attr("x",n+o).attr("y",n-o).text(r=>s.getShowData()?`${r.label} [${r.value}]`:r.label);const U=Math.max(...E.selectAll("text").nodes().map(r=>(r==null?void 0:r.getBoundingClientRect().width)??0)),X=h+e+n+o+U,L=((O=j.node())==null?void 0:O.getBoundingClientRect().width)??0,Z=h/2-L/2,q=h/2+L/2,P=Math.min(0,Z),B=Math.max(X,q)-P;v.attr("viewBox",`${P} 0 ${B} ${p}`),ut(v,p,B,l.useMaxWidth)},"draw"),zt={draw:Rt},Pt={parser:bt,db:V,renderer:zt,styles:Et};export{Pt as diagram};
