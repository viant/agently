...
func (r *Registry) InjectVirtualAgentTools(agents []*agent.Agent, domain string) {
	...
	for _, ag := range agents {
		...
		r.virtualTimeout[toolID] = support
	}
}
...
func newToolCacheEntry(def *llm.ToolDefinition, mcpDef mcpschema.Tool, support timeoutSupport) *toolCacheEntry {
	if def == nil {
		return nil
	}
	...
	for _, s := range servers {
		...
		for _, t := range tools {
			...
			if def := llm.ToolDefinitionFromMcpTool(&t); def != nil {
				...
				r.mu.Lock()
				if entry := newToolCacheEntry(def, t, support); entry != nil {
					r.cache[disp] = entry
				}
				r.mu.Unlock()
			}
	...
	if svc := serverFromPattern(pattern); svc != "" {
		...
		for _, t := range tools {
			...
			if tmatch.Match(pattern, full) {
				...
				def.Name = full
				support := ensureTimeoutMs(def)
				entry := newToolCacheEntry(def, t, support)
				if entry == nil {
					continue
				}
				defCopy := entry.def
	...
	for _, t := range tools {
		if strings.TrimSpace(t.Name) == strings.TrimSpace(method) {
			...
			if tool != nil {
				...
				tool.Name = fullSlash
				support := ensureTimeoutMs(tool)
				entry := newToolCacheEntry(tool, t, support)
...
func (r *Registry) injectOrchestratorVirtualTool() {
	...
	r.virtualTimeout[def.Name] = support
}
